...
var source_card: CardModel = null
...
var _has_timeout_condition:
	get(): return source_card != null or total_duration_seconds != -1
...
func bind_to_card(card: CardModel, destroy_card_if_removed: bool = true) -> Shield:
	...
	source_card = card
	source_card.exited_play.connect(on_source_card_exited)
	_destroy_card_if_removed = destroy_card_if_removed
	return self

func set_duration(duration_seconds: float) -> Shield:
	if source_card: push_error("Don't add duration to a shield bound to a card")
...
func on_source_card_exited() -> void:
	remove_from_creature_internal()
...
func remove_from_creature() -> void:
	if _destroy_card_if_removed:
		CardCommand.remove_from_play(source_card)
	remove_from_creature_internal()
...
func get_time_left() -> float:
	...
	if source_card: return source_card.time_left_in_play
