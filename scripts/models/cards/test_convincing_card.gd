...
func on_play(card_play: CardPlay) -> void:
	EffectCommand.new(ConvincingEffect, card_play.target, convincing_amount).execute()
	#EffectCommand.new(ConvincingEffect, card_play.target, convincing_amount).from_card(self).execute()
	EffectCommand.new(ConvincingEffect, card_play.target, convincing_amount).with_duration(5.0).execute()
